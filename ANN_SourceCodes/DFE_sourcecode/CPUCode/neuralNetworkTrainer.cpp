																	...
																	generalizationSetMSE(0)																	
{
	//create delta lists
	//--------------------------------------------------------------------------------------------------------
	deltaInputHidden = new( NetVariableType*[NN->nInput + 1] );
	for ( int i=0; i <= NN->nInput; i++ ) 
	{
		deltaInputHidden[i] = new (NetVariableType[NN->nHidden]);
		for ( int j=0; j < NN->nHidden; j++ ) deltaInputHidden[i][j] = 0;		
	}

	deltaHiddenOutput = new( NetVariableType*[NN->nHidden + 1] );
	for ( int i=0; i <= NN->nHidden; i++ ) 
	{
		deltaHiddenOutput[i] = new (NetVariableType[NN->nOutput]);			
		for ( int j=0; j < NN->nOutput; j++ ) deltaHiddenOutput[i][j] = 0;		
	}

	//create error gradient storage
	//--------------------------------------------------------------------------------------------------------
	hiddenErrorGradients = new( NetVariableType[NN->nHidden + 1] );
	for ( int i=0; i <= NN->nHidden; i++ ) hiddenErrorGradients[i] = 0;
	
	outputErrorGradients = new( NetVariableType[NN->nOutput + 1] );
...
********************************************************************/
void neuralNetworkTrainer::setTrainingParameters( NetVariableType lR, NetVariableType m, bool batch )
{
	learningRate = lR;
	momentum = m;
	useBatch = batch;
...
********************************************************************/
void neuralNetworkTrainer::setStoppingConditions( int mEpochs, NetVariableType dAccuracy )
{
	maxEpochs = mEpochs;
...
********************************************************************/
inline NetVariableType neuralNetworkTrainer::getOutputErrorGradient( NetVariableType desiredValue, NetVariableType outputValue)
{
...
********************************************************************/
NetVariableType neuralNetworkTrainer::getHiddenErrorGradient( int j )
{
	//get sum of hidden->output weights * output error gradients
	NetVariableType weightedSum = 0;
...
void neuralNetworkTrainer::trainNetwork( trainingDataSet* tSet )
{
	...
	lastEpochLogged = -logResolution;
		...
		if ( loggingEnabled && logFile.is_open()){
			logFile	<< endl << " Neural Network Training Starting: " << endl
					<< "==========================================================================" << endl
					<< " LR: " << learningRate << ", Momentum: " << momentum << ", Max Epochs: " << maxEpochs << endl
					<< " " << NN->nInput << " Input Neurons, " << NN->nHidden << " Hidden Neurons, " << NN->nOutput << " Output Neurons" << endl
					<< "==========================================================================" << endl << endl;
	...
	{			
		...
		NetVariableType previousTAccuracy = trainingSetAccuracy;
		NetVariableType previousGAccuracy = generalizationSetAccuracy;


		    std::clock_t start;
		    NetVariableType duration;
		...
		runTrainingEpoch( tSet->trainingSet );
			...
			duration = ( std::clock() - start ) / (NetVariableType) CLOCKS_PER_SEC;
...
//if ( ceil(previousTAccuracy) != ceil(trainingSetAccuracy) || ceil(previousGAccuracy) != ceil(generalizationSetAccuracy) ) 
		{	
			...
			cout << " GSet Acc:" << generalizationSetAccuracy << "%, MSE: " << generalizationSetMSE ;
			if (TIME_DURATION_CHECK){		cout << " Duration: "<< duration << endl;}
			else{							cout << endl;}
		}
...
{
	//incorrect patterns
	NetVariableType incorrectPatterns = 0;
...
********************************************************************/
void neuralNetworkTrainer::backpropagate( NetVariableType* desiredOutputs )
{		
