...
bool dataReader::loadDataFile( const char* filename, int nI, int nT )
{
	...
	if ( inputFile.is_open() )
	{
		...
		//shuffle full data vector
		if (DO_random_shuffle_WHEN_LOADING_DS){
...
void dataReader::processLine( string &line )
{
	...
	NetVariableType* pattern = new NetVariableType[nInputs];
	NetVariableType* target = new NetVariableType[nTargets];
...
********************************************************************/
void dataReader::setCreationApproach( int approach, NetVariableType param1, NetVariableType param2 )
{
	//static
	if ( approach == STATIC )
	{
	...
	else if ( approach == WINDOWING )
	{
		...
		if ( param1 < data.size() && param2 <= param1)
		{
			...
			//number of sets
			numTrainingSets = (int) ceil( (NetVariableType) ( trainingDataEndIndex - windowingSetSize ) / windowingStepSize ) + 1;
		}			
	}
